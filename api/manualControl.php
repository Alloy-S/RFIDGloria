...
if ($_SERVER['REQUEST_METHOD'] === "POST") {
    ...
    if (htmlspecialchars($_POST['method']) === "complete") {
        $idMurid = htmlspecialchars($_POST['id_murid']);

        // echo $studentId;
        $stmt = $conn->prepare("SELECT * FROM live_view WHERE murid_id=:studentId");
        $stmt->execute([":studentId" => $idMurid]);
        ...
        echo "404";
        return;
    } else if (htmlspecialchars($_POST['method']) === "remove") {
        $idMurid = htmlspecialchars($_POST['id_murid']);

        // echo $studentId;

        $stmt = $conn->prepare("DELETE FROM live_view WHERE murid_id=:studentId;");
        $stmt->execute([":studentId" => $idMurid]);
