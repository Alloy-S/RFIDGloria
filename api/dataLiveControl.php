...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $result_query = "SELECT a.murid_id AS id_murid, CONCAT(a.grade,a.class) AS kelas, a.student_name AS murid, b.jenis_mobil AS jenis_mobil,b.plat_mobil AS plat_mobil, (CASE WHEN UNIX_TIMESTAMP(CURRENT_TIMESTAMP) - UNIX_TIMESTAMP(a.entry_date) < 60 THEN CONCAT('Waiting for ', FLOOR((UNIX_TIMESTAMP(CURRENT_TIMESTAMP) - UNIX_TIMESTAMP(a.entry_date))), ' second') WHEN UNIX_TIMESTAMP(CURRENT_TIMESTAMP) - UNIX_TIMESTAMP(a.entry_date) >= 60 AND UNIX_TIMESTAMP(CURRENT_TIMESTAMP) - UNIX_TIMESTAMP(a.entry_date) < 3600 THEN CONCAT('Waiting for ', FLOOR((UNIX_TIMESTAMP(CURRENT_TIMESTAMP) - UNIX_TIMESTAMP(a.entry_date)) / 60), ' minute') WHEN UNIX_TIMESTAMP(CURRENT_TIMESTAMP) - UNIX_TIMESTAMP(a.entry_date) >= 3600 THEN CONCAT('Waiting for ', FLOOR((UNIX_TIMESTAMP(CURRENT_TIMESTAMP) - UNIX_TIMESTAMP(a.entry_date)) / 3600), ' hour') ELSE 'Has arrived' END) AS status FROM live_view AS a JOIN db_kendaraan AS b ON a.UID = b.rfid_tag ";

    if ($grade == 'sd') {
        $result_query .= "AND (a.grade IN (" . implode(',', $sd) . "))";
            
    } elseif ($grade == 'smp') {
        $result_query .= "AND (a.grade IN (" . implode(',', $smp) . "))";
        
    } elseif ($grade == 'tk') {
        $result_query .= "AND ((a.grade NOT IN (" . implode(',', $sd) . ")) AND (a.grade NOT IN (" . implode(',', $smp) . ")))";
    }
    
    $result_query .= "ORDER BY a.entry_date DESC;";
